...
def plot_spectrum(specup:np.ndarray, specdn:np.ndarray=None, colors:Dict[str, str]=None, figsize=(30, 20), linewidth:float=1.2,
                      mzmax:Union[float, int]=None, intmax:Union[float, int]=2, intscale:Literal["Relative", "Absolute"]="Relative", plothres:float=0.01,
                      peak_colors: Union[dict[float, str], dict[str, dict[float, str]]]=None, score2disply:str = "modified_cosine", title_prefix:str=None,
                      figdir:str=None, figname:str=None, dpi:int=600):
    # singleplot=False
    # if specdn is None:
    #     if not (("mz" in specup) and ("intensity" in specup)):
    #         raise ArgumentError("The argument 'mz' and 'intensity' should be a key of specup")
    #     singleplot=True
    # else:
    #     if not (("mz" in specup) and ("intensity" in specup) and ("mz" in specdn) and ("intensity" in specdn)):
    #         raise ArgumentError("The argument 'mz' and 'intensity' should be a key of specup and specdn")
    #     mzdn, intdn = specdn["mz"], specdn["intensity"]
    # mzup, intup = specup["mz"], specup["intensity"]

    singleplot=False
    if specdn is None:
        if not (specup.shape[1] == 2):
            raise ValueError("The argumnet 'specup' should be given as a [None, 2]-dimensional np.ndarray")
        singleplot=True
    else:
        if not ((specup.shape[1] == 2) or (specdn.shape[1] == 2)):
            raise ValueError("The argumet 'specup' and 'specdn' should be given as a [None, 2]-dimensional np.ndarray")
        mzdn, intdn = specdn[:,0], specdn[:,1]
    mzup, intup = specup[:,0], specup[:,1]

    if mzmax is None:
        if singleplot:
            mzmax_plot = np.max(mzup)*1.2
        else:
            mzmax_plot = np.max([np.max(mzup), np.max(mzdn)]) * 1.2
    else:
        mzmax_plot = mzmax

    if intscale.lower() == "relative":
        if singleplot:
            if intup.max() > 0:
                intup_plot = intup/np.max(intup)
                mzup_plot = mzup.copy()
            else:
                raise ValueError("The maximum intensity of 'specup' is not positive")
        else:
            if not intup.max() > 0:
                raise ValueError("The maximum intensity of 'specup' is not positive")
            elif not intdn.max() > 0:
                raise ValueError("The maximum intensity of 'specdn' is not positive")
            else:
                intup_plot = intup/np.max(intup)
                intdn_plot = intdn/np.max(intdn)
                mzup_plot = mzup.copy()
                mzdn_plot = mzdn.copy()

        intmax_plot = intmax
        if singleplot:
            yticks = list(np.arange(0, intmax_plot + 1, 1))
        else:
            yticks = list(np.arange(-intmax_plot, intmax_plot+1, 1))
    else:
        if singleplot:
            if intup.max() > 0:
                intmax_plot = np.max(intup)*1.2
                intup_plot = intup.copy()
                mzup_plot = mzup.copy()
            else:
                raise ValueError("The maximum intensity of 'specup' is not positive")
        else:
            if not intup.max() > 0:
                ...
                intmax_plot = np.max([np.max(intup), np.max(intdn)])*1.2
                intup_plot = intup.copy()
                intdn_plot = intdn.copy()
                mzup_plot = mzup.copy()
                mzdn_plot = mzdn.copy()

    mzup_plot = mzup_plot[np.where(intup_plot > plothres)]
    intup_plot = intup_plot[np.where(intup_plot > plothres)]
    if not singleplot:
        mzdn_plot = mzdn_plot[np.where(intdn_plot > plothres)]
        intdn_plot = intdn_plot[np.where(intdn_plot > plothres)]

    if singleplot:
        ylim_plot = [0, intmax_plot]
    else:
        ylim_plot = [-intmax_plot, intmax_plot]
    xlim_plot = [0.0, np.ceil(mzmax_plot)]

    if colors is None:
        colors = {"up": "#0073CF", "dn": "#FF0000"}

    if not singleplot:
        spec1 = array2spec(mzs=mzup_plot, ints=intup_plot)
        spec2 = array2spec(mzs=mzdn_plot, ints=intdn_plot)
        scores = score_similarity(spec1=spec1, spec2=spec2, method=score2disply)
        if title_prefix is not None:
            title_plot = f"{title_prefix} | cosine={scores[score2disply]:.4f}"
        else:
            title_plot = f"cosine={scores[score2disply]:.4f}"
    else:
        if title_prefix is not None:
            title_plot = title_prefix
        else:
            title_plot = ""

    panels, peakidxs, mzs, ints = [], [], [], []
    fig = plt.figure(num=1, figsize=figsize)
    _, stemlines1, _ = plt.stem(mzup_plot, intup_plot, linefmt=colors["up"], markerfmt='', basefmt=" ")
    for cnt, (mz, it) in enumerate(list(zip(mzup_plot, intup_plot))):
        panels.append("Up")
        peakidxs.append(cnt+1)
        mzs.append(mz)
        ints.append(it)

    try:
        stemlines1.set_linewidth(linewidth)
    except Exception:
        pass

    if not singleplot:
        _, stemlines2, _ = plt.stem(mzdn_plot, -intdn_plot, linefmt=colors["dn"], markerfmt='', basefmt=" ")
        for cnt, (mz, it) in enumerate(list(zip(mzdn_plot, intdn_plot))):
            panels.append("Down")
            peakidxs.append(cnt + 1)
            mzs.append(mz)
            ints.append(it)
        try:
            stemlines2.set_linewidth(linewidth)
        except Exception:
            pass

    if not (peak_colors is None):
        mzs_again, ints_again, colors_again = [], [], []
        for m, c in peak_colors["up"]:
            idxs = np.where(mzup_plot == m)[0]
            if len(idxs) > 0:
                mzs_again.extend([m] * len(idxs))
                ints_again.extend(list(intup_plot[idxs]))
                colors_again.extend([c] * len(idxs))

        if not singleplot:
            for m, c in peak_colors["dn"]:
                idxs = np.where(mzdn_plot == m)[0]
                if len(idxs) > 0:
                    mzs_again.extend([m] * len(idxs))
                    ints_again.extend(list(-intdn_plot[idxs]))
                    colors_again.extend([c] * len(idxs))

        if len(mzs_again) > 0:
            for m, i, c in list(zip(mzs_again, ints_again, colors_again)):
                _, stemlines3, _ = plt.stem([m], [i], linefmt=c, markerfmt='', basefmt=" ")
                try:
                    stemlines3.set_linewidth(linewidth)
                except Exception:
                    pass

    plt.axhline(0, color='black', linewidth=1)
    plt.xlabel("m/z", fontdict={"fontsize": 14, "fontweight": "bold"})
    plt.ylabel(intscale + " Intensity", fontdict={"fontsize": 14, "fontweight": "bold"})
    plt.xlim(xlim_plot)
    plt.ylim(ylim_plot)
    if intscale.lower() == "relative":
        plt.yticks(yticks)
    ...
    for figformat in ["png", "pdf"]:
        if figdir is None:
            if figname is None:
                figfile = os.path.join(os.getcwd(), "Unknown." + figformat)
            else:
                figfile = os.path.join(os.getcwd(), figname + "." + figformat)
        else:
            if figname is None:
                figfile = os.path.join(figdir, "Unknown." + figformat)
            else:
                figfile = os.path.join(figdir, figname + "." + figformat)
        os.makedirs(os.path.dirname(figfile), exist_ok=True)
        plt.savefig(figfile, dpi=dpi)
        print("figure saved at : " + figfile)
    plt.close(fig)

    filename = os.path.splitext(os.path.basename(figfile))[0]
    peakmetafile = os.path.join(os.path.dirname(figfile), filename + "_peakmeta.txt")
    df = pd.DataFrame.from_dict({"panels": panels, "peakidxs": peakidxs, "mzs": mzs, "ints ("+intscale+")": ints})
    df.to_csv(peakmetafile, sep="\t", index=False)
    print("Peak meeta saved at : " + peakmetafile)
...
__all__ = ["str2array", "array2spec", "spec2array", "array2str", "spec2str", "str2spec", "normalizeSpectrum", "sortSpectrum",
           ...
           "batch_drop_msms_precursor", "batch_clean", "batch_normalize", "batch_spectral_entropy", "batch_neutral_loss", "plot_spectrum"]